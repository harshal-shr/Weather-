...
async function searchWeather() {
    ...
    try {
        ...
        console.log('Fetching:', currentUrl); // Debug
        
        const currentResponse = await fetch(currentUrl);
        
        if (!currentResponse.ok) {
            if (currentResponse.status === 404) {
                throw new Error('City not found! Try: Mumbai, London, New York, Tokyo');
        ...
        console.log('Fetching:', forecastUrl); // Debug
        
        const forecastResponse = await fetch(forecastUrl);
        const forecastData = await forecastResponse.json();
        console.log('Forecast Data:', forecastData); // Debug
...
function displayForecast(data) {
    ...
    // Get unique days (API returns every 3 hours)
    const dailyData = [];
